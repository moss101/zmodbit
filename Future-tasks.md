...
2. Substrate adapter on Firecracker: adapter code COMPLETE in this pass (M8.3 → WIRED, commit chain `5d3ddf6`..HEAD) — `crates/sandbox::substrate`: FirecrackerSpec (secret-free VM config), typed FirecrackerApi client, VM lifecycle controller (spawn → wait API socket → machine-config/boot-source/rootfs/vsock PUTs → InstanceStart → Running poll → shutdown/Drop-kill), host vsock link (`CONNECT <port>`/`Ok` handshake over the VMM UDS), GuestBootstrap contract (generic rootfs + tmpfs-only provisioning env; provision key never in images/configs — asserted by test), Linux AF_VSOCK guest listener + `MODBIT_GUEST_VSOCK_PORT` transport option in `modbit-guest` (dev TCP and production vsock share one connection body). Proven: 4-test substrate suite against a fixture VMM socket + REAL subprocess lifecycle (boot to Running, fault mapping, missing-binary typed failure, vsock handshake/refusal, secret-free spec). REMAINING (environmental, agent-unprovable): real-guest conformance — E2E-017's actual-MicroVM boot + `conformance_suite` through a live VM — requires a Linux/KVM host with firecracker + kernel/rootfs artifacts; operator-run conformance per the phase exit.
3. Credential broker: COMPLETE in this pass (M8.6 → E2E_PROVEN, commit chain through `4c5aaae`) — `crates/secrets` (canonical effects-security owner, stub replaced): opaque SecretRef registration, short-lived CredentialLease bound to task+generation+scope, execution-time materialization failing closed on Expired/Revoked/TaskMismatch/Fenced/ScopeDenied, task-scoped emergency revocation (compromised guest loses only its own authority), value-free audit trail, fingerprint redaction; E2E proves a lease materializing into a REAL child env on the REAL guest with a synthetic fixture secret. REMAINING (environmental): guest egress ENFORCEMENT (firewall/netns at the substrate tap) binds at real-guest qualification with item 2.
4. Local→cloud checkpoint handoff: ATTACH CONTRACT complete in this pass (M8.7 → WIRED) — `crates/git` exports/imports self-contained git bundles of modbit-namespace snapshot refs; `crates/checkpoint::cloud_attach` defines the transport-agnostic `CheckpointHandoffBundle` (env handoff + snapshot handle + bundle bytes + digest) with export (local dirty state → transferable bundle; user's branch/worktree/index untouched) and attach (cloud side: validation, transfer digest before git, task provenance binding, restore, EXACT reconstruction proven by tree digest). Proven on real repos: dirty+untracked content survives the trip; corrupt transfers and cross-task attaches are refused. ROUTING CLOSED in this pass (commit chain through the M8.7-routing commit): surface.proto field 37 `ImportCheckpointCommand`/`CheckpointAttachView` (TS bindings regenerated, zero drift, typecheck green); cloud-api gained the tenant-bearer POST `/checkpoint` endpoint relaying the typed RPC through the gateway; the cloud worker takes `MODBIT_REPO_ROOT` and `CoreServices::import_checkpoint` binds the attach contract to the worker's repository. E2E proves the FULL route: local dirty repo → export → relay → attach with the tree-digest EXACT receipt → reconstructed files verified on the worker; cross-task provenance refusal typed, no receipt. M8.7 → E2E_PROVEN.
5. CLOSED in this pass: cloud browser remote stream/CDP relay (M8.8 → E2E_PROVEN) — cloud-api gained the tenant-authed `/browser` endpoint relaying the typed GetBrowserView(34)/SetBrowserLease(35) RPCs through the gateway to the tenant worker's BrowserHost (the ONLY browser path — SurfaceProtocol + Core dispatch; no second runtime, no browser in the cloud api). E2E (`apps/cloud-api/tests/cloud_api_e2e.rs::cloud_browser_relay_view_lease_and_tenant_isolation`, real HTTP + real gateway + real worker + REAL Chromium): live PNG + page + lease agent→user→agent through the cloud plane; second tenant fails closed at its own plane and never observes tenant-1's browser state.
6. Managed Postgres + S3 bindings — S3 BINDING CLOSED in this pass: the ObjectStore seam in `crates/event-store` (ONE contract, three interchangeable backends: SqliteObjectStore = today's default semantics, LocalFsObjectStore = content-addressed atomic files, S3ObjectStore = minimal SigV4 signer over the tree's own hmac/sha2), the shared contract suite (round trip, digest fail-closed, range clamp, replace, delete, tenant/task namespace isolation, retention metadata) green on ALL THREE — the S3 backend against a REAL S3-compatible server (MinIO via docker, bucket created through the signer itself); RuntimeStore::with_object_store delegates NEW output-ref payloads to the backend (SQL keeps preview+metadata only, asserted) with digest-verified reads and legacy inline-row fallback; default store is byte-for-byte today's behavior. POSTGRES ADAPTER CLOSED in this pass too: `crates/event-store::runtime_pg` — PgRuntimeStore mirrors the SQLite RuntimeStore method-for-method over the runtime surface (output refs content-addressed with bounded previews, durable background operations, durable tool call/result pairing) on a REAL local Postgres (docker postgres:16-alpine); dialect mapping documented (?N→$N, upserts via ON CONFLICT, BYTEA payloads); NUL-in-preview parity deviation documented (PG TEXT refuses what SQLite tolerated). Item 6 REMAINING (both halves agent-complete): none — the MANAGED cloud boundaries (RDS/AWS S3 proper: credentials + TLS) stay operator-gated.
