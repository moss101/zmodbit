...
| M3.1 exact/regex/path surfaces: `context.query` gains mode=fused|exact|regex|path over the indexed corpus (journal-refreshed, retrieval-evidence recording); E2E proves the regex surface searches the live index (mutation-checked) | commit `727814a`; `docs/evidence/phase3-1..4-*.log` harness patterns |
