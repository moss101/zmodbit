//! Daemon-driven context-compaction E2E (Future-tasks.md Phase 2 item 2):
//! the REAL `modbit-core` daemon, real scheduler, real git worktree, real
//! execd broker, real `fs.read` effector — with MODBIT_MAX_INPUT_TOKENS
//! pinned tiny so the second (repair-turn) request MUST compact before
//! the invoke. A body-capturing scripted fixture proves the wire: the
//! tool result travels truncated with linkage intact, and the durable
//! event store carries the `compaction_applied` run event (asserted by
//! reading the daemon's real SQLite database).
...
const BIG: &str = "lorem ipsum dolor sit amet consectetur adipiscing elit sed do eiusmod\n";

fn big_file_fixture(tag: &str) -> PathBuf {
    ...
    // ~8 KB of repetitive content: ~2000 estimated tokens, far over the
    // pinned 700-token budget once the prompt rides along.
    std::fs::write(root.join("big.txt"), BIG.repeat(150)).unwrap();
...
/// Body-capturing scripted model fixture (one SSE body per connection).
...
fn spawn_core(
    ...
    let mut child = command
        ...
        .env("MODBIT_MAX_INPUT_TOKENS", max_input_tokens)
...
fn wait_ready_for_review(daemon: &str, task_id: &str) {
    ...
    loop {
        ...
        if state == pb::TaskStatus::Failed as i32 || Instant::now() > deadline {
            panic!("compaction e2e: task did not reach ReadyForReview (state {state})");
        }
        std::thread::sleep(Duration::from_millis(200));
    }
}

/// The pinned budget forces the repair-turn request to compact: the tool
/// result travels truncated (linkage intact), the request stays
/// well-formed, the per-model settings ride the body, and the durable
/// store records `compaction_applied`.
#[test]
fn over_budget_daemon_compacts_and_records_the_event() {
    let _guard = E2E_LOCK.lock().unwrap_or_else(|p| p.into_inner());
    let repo = big_file_fixture("cp");
    let worktrees = tempdir("cw");
    let (model, bodies) = spawn_model_fixture(vec![
        tool_call_turn("c1", "fs.read", r#"{"path":"big.txt"}"#),
        text_turn("read it"),
    ]);
    let (mut core, daemon, db_path) = spawn_core(&repo, &worktrees, model, "700");
    ...
    // Wire proof: the repair-turn request compacted the tool result but
    // kept the provider contract (roles + call-id linkage).
    ...
    assert_eq!(
        ...
        "compaction preserves the role sequence"
    );
    let tool = &messages[3];
    assert_eq!(tool["tool_call_id"], "c1", "linkage survives compaction");
    ...
    let count: i64 = conn
        ...
        .unwrap();
    assert!(count >= 1, "compaction_applied durable in the run plane");
    let (affected, reclaimed, digest): (i64, i64, String) = conn
        .query_row(
            "SELECT json_extract(payload_inline, '$.affected_messages'),
                    json_extract(payload_inline, '$.reclaimed_tokens'),
                    json_extract(payload_inline, '$.manifest_digest')
             FROM events WHERE event_type = 'compaction_applied' LIMIT 1",
            [],
    ...
    assert!(affected >= 1);
    assert!(reclaimed > 0);
    assert_eq!(digest.len(), 64, "sha256 manifest digest: {digest}");
