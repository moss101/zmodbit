//! Daemon-driven media E2E (M2.10, docs/25): a REAL PNG read through
//! the production fs.read tool returns a typed MediaEnvelope (provenance
//! digest, byte length, content-addressed object) instead of lossy
//! binary — asserted on the fixture-captured tool result that rides the
//! model conversation; the artifact exists in the worktree's object
//! store; text reads stay plain.
...
fn media_fixture(tag: &str) -> (PathBuf, Vec<u8>) {
    ...
    // A REAL minimal PNG (magic bytes + IHDR/IEND) from the crate's own
    // generator.
    let png = modbit_tools::media::minimal_png();
    std::fs::write(root.join("logo.png"), &png).unwrap();
    std::fs::write(root.join("notes.txt"), "plain text stays plain\n").unwrap();
...
fn wait_ready_for_review(daemon: &str, task_id: &str) {
    ...
    loop {
        ...
        if state == pb::TaskStatus::Failed as i32 || Instant::now() > deadline {
            panic!("media e2e: task did not complete (state {state})");
        }
        std::thread::sleep(Duration::from_millis(150));
    }
}

/// A real PNG read through the production fs.read returns the typed
/// MediaEnvelope with provenance; a text read stays plain.
#[test]
fn png_read_returns_media_envelope_with_provenance() {
    let _guard = E2E_LOCK.lock().unwrap_or_else(|p| p.into_inner());
    let (repo, png) = media_fixture("ma");
    let worktrees = tempdir("mw");
    let (model, bodies) = spawn_model_fixture(vec![
        tool_call_turn("c1", "fs.read", r#"{"path":"logo.png"}"#),
        tool_call_turn("c2", "fs.read", r#"{"path":"notes.txt"}"#),
        text_turn("read both"),
    ]);
    ...
    assert!(bodies.len() >= 3, "fixture captured all turns");
    // Request N carries the tool results of turns < N keyed by call id.
    let tool_result = |turn: usize, call_id: &str| -> serde_json::Value {
        let messages = bodies[turn]["messages"].as_array().unwrap();
        let tool = messages
            .iter()
            .find(|m| m["role"] == "tool" && m["tool_call_id"] == call_id)
            .unwrap_or_else(|| panic!("tool result {call_id} on turn {turn}"));
    ...
    let digest = media["sha256"].as_str().unwrap();
    assert_eq!(digest.len(), 64, "sha256 provenance: {digest}");
    // The digest is the REAL sha256 of the file bytes.
    {
        use sha2::Digest;
    ...
    assert!(text_result.get("media").is_none(), "text is not media");
    assert_eq!(text_result["content"], "plain text stays plain\n");
