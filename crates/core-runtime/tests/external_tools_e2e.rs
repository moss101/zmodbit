...
fn external_tools_round_trip_over_the_real_daemon() {
    ...
    let script = [
        ...
        tool_call_turn(
            "c2",
            "external.call",
            r#"{"server":"fixture","tool":"echo","arguments":{"text":"modbit"}}"#,
        ),
        text_turn("finished"),
    ];
    ...
    let fixture_bin =
        PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("../../target/debug/mcp-fixture-server");
    ...
    let created = send(pb::surface_request::Request::CreateTask(
