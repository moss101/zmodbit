//! Cloud transport envelope (docs/24): the gateway relays opaque framed
//! payloads between authenticated tenants and workers. The envelope is
//! the ONLY thing the gateway parses — the wrapped bytes are the
//! SurfaceProtocol, which the gateway never interprets (boundary, not
//! brain). Canonical owner: the protocol crate — gateway, worker and api
//! share these types from here.
